...
int main() {
    ...
    // Boot RAM has none of the initialized gameplay structures, so it must
    // never produce an IPC payload.
    assert(!capture().has_value());
    // Title-like RAM can have the timer flag set, but lacks a valid map and
    // complete party data. It is likewise suppressed rather than clearing a
    // previous River XMB snapshot.
    put(0xd732, 1);
    assert(!capture().has_value());
    ...
    // A normal overworld snapshot is the first state eligible for publication.
    const auto overworldPayload = nlohmann::json::parse(BMMQ::RiverXmbIntegration::telemetryJson(*data));
    assert(overworldPayload["action"] == "telemetry-update");
    assert(overworldPayload["telemetry"]["location"] == "Viridian City");
    ...
    put(0xd057, 0); put(0xd11d, 0);
    put(0xd163, 7); assert(!capture().has_value());
    put(0xd163, 6); put(0xd16a, 0); assert(!capture().has_value());
    put(0xd16a, 255); put(0xda44, 60); assert(!capture().has_value());
    put(0xda44, 4); word(0xd16c, 401); assert(!capture().has_value());
    put(0xd16c, 12); put(0xd732, 0); assert(!capture().has_value());
    put(0xd732, 1); put(0xd35e, 255); assert(!capture().has_value());
