...
struct microcode {
	...
	std::size_t get_return_index() {
		...
		if constexpr (cur == N) {
			...
			else return get_return_index<N, cur + 1, R + 1>();
		}
	}
	...
	return_type invoke_all(argument_type& args) const {
		return_type results{};
		invoke_all(func_list, results, args);
		return results;
	}


	template<std::size_t If, std::size_t Ir, auto F = get<If>(), class... args>
	void invoke_one() {
	}
